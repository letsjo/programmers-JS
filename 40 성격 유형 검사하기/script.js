...
function solution(survey, choices) {
    ...
    for (let i = 0; i < survey.length; i++) {
        if (score[choices[i]] >= 0) {
            point.set(survey[i][0], point.get(survey[i][0]) + score[choices[i]]);
        } else {
            point.set(survey[i][1], point.get(survey[i][1]) + Math.abs(score[choices[i]]));
        }
    }

    character.forEach((type) => {
        if (point.get(type[0]) >= point.get(type[1])) {
