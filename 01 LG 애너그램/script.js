...
// 이는 숫자에도 적용할 수 있는데, 예를 들어 1814와 1481은 애너그램입니다. 1814의 각 자릿수 위치를 바꾸어 1481을 만들 수 있기 때문입니다. 단, 두 숫자의 자릿수는 같아야 합니다. 예를 들어 1201과 112의 경우, 1201 → 0112 → 112가 되지만, 1201은 네 자리 숫자, 112는 세 자리 숫자이므로 애너그램 관계가 아닙니다.
...
function solution(arr) {
  const arrAnagram = arr.map((unit,idx)=>{
    return (""+unit).split('').sort().join('');
  })
  return [...new Set(arrAnagram)].length;
}